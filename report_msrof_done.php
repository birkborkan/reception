...
if(!empty($s_date))
    {
      ...
      $get_opr=mysqli_query($conn,"select msrof_byan,date,sum(msrof_price),count(msrof_id),msrof_price from msrof 
       where convert(date,Date) = '$s_date' GROUP BY msrof_byan  order by msrof_id asc") or die(mysql_error());
    ...
    if(!empty($s_date1) or !empty($s_date2))
    {
        ...
        $get_opr=mysqli_query($conn,"select msrof_byan,date,sum(msrof_price),count(msrof_id),msrof_price from msrof 
        where convert(date,Date) between '$s_date1' and '$s_date2' GROUP BY msrof_byan  order by msrof_id asc") or die(mysql_error());
...
<?php
 ...
 ?>
    <div id="reporttitle"  style='float:left;width:50%;direction: rtl;font-size:1.2em;smargin-bottom:10px;border-bottom-style: groove'> تاريخ الطباعة <?php echo $date_;?></div> 
     <div id="reporttitle"  style='float:left;width:50%;direction: rtl;text-align:center;font-size:1.2em;border-bottom-style: groove;margin-bottom:10px;'> تقرير عن المصروفات    <?php echo $text;?></div>
      <table class="table table-bordered" id="dataTable" width="50%" style='font-size:11px;text-align:right;' cellspacing="0">
          ...
          <th>#</th>
               ...
               اجمالي تكلفة المصروفات : '.number_format($sum).'  </div>
               ';
               echo '
               <div style="text-align:left;font-size:1.2em;margin-right:5%">
               عدد  المصروفات : '.$count.'  </div>
               ';
               ?>
               </div>
