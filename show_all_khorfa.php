<?php
 ...
 @$date = date('Y-m-d'); 
 if( $user_status == 1){
 $select  = mysqli_query($conn,"select * from khorfa where convert(date,Date) = '$date'  order by khorfa_id desc");
 } else {
 $select  = mysqli_query($conn,"select * from khorfa where convert(date,Date) = '$date' and user_id = '$user_id' order by khorfa_id desc");
 } ?>

<div style='text-align:right;'>
 <span>بحث:</span>
 <input type='text'id='search_text'class='form-control' onkeydown='search_new("search_text","show_all_khorfa_search.php","stay_id",,<?php echo $user_id; ?>,<?php echo $user_status; ?>);' onkeyup = 'search_new("search_text","show_all_khorfa_search.php","stay_id",<?php echo $user_id; ?>,<?php echo $user_status; ?>)'/>
 </div>
 <div id = 'stay_id'>
