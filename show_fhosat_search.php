...
//  echo "uid:".$user_id;
 ...
 pa_name,sum(fhs_price),count(fatora_num),fatora_num,fhs_date,user_id from fhs_op
  where (pa_name like '%$search_text%' or fatora_num like '%$search_text%') and convert(date,Date) = '$date' GROUP BY fatora_num order by fhs_op_id desc");
 }
  else 
  {
 $get_opr  = mysqli_query($conn,"select
 pa_name,sum(fhs_price),count(fatora_num),fatora_num,fhs_date from fhs_op
  where (pa_name like '%$search_text%' or fatora_num like '%$search_text%') and  convert(date,Date) = '$date' and user_id = '$user_id' GROUP BY fatora_num   order by fhs_op_id desc");
 }
