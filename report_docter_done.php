...
if(!empty($s_date))
    {
      $get_opr=mysqli_query($conn,"select * from mogabla 
       where convert(date,Date) = '$s_date' and doc_id ='$doc_id'   order by mog_id asc") or die(mysql_error());
    ...
    if(!empty($s_date1) or !empty($s_date2))
    {
        ...
        $get_opr=mysqli_query($conn,"select * from mogabla 
        where convert(date,Date) between '$s_date1' and '$s_date2'and doc_id ='$doc_id'  order by mog_id asc") or die(mysql_error());
